...
<h1>ORDER CREATE</h1>
<p>{{ $game_option['manth'] }}/{{ $game_option['day'] }}({{ $game_option['day_of_week'] }}) {{ $game_option['start_time'] }}~のオーダー作成ページ</p>
    <section>
        <div>
            <form method="POST" action="{{route('baseball.game_detail.store')}}">
                ...
                <input type="hidden" name="dating" value="{{ $game_option['dating'] }}">
                <input type="hidden" name="game_detail_id" value="{{ $game_option['id'] }}">
